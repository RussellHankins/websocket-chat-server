...
bool chatcommand_addchatroom::processmessage(char first_letter,message *received_message,chatclient *client)
{
	...
	if (parameter_success) {
		...
		new_chatroom->clients.add(client); // Add this user to the chat room.
		...
		new_message = chatroomcreated(parameters_parsed.long_parameters[0],new_chatroom->chatroomid,parameters_parsed.string_parameters[1],new_chatroom->info(true));
		...
		new_message = chatroomcreated(parameters_parsed.long_parameters[0],new_chatroom->chatroomid,parameters_parsed.string_parameters[1],new_chatroom->info(false));
